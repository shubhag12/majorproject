...
// //  account0='0xC615F9625129C028CA62e3D36B6dFa10b6cF3266';
...
async function renderPageContent() {
  await AssetTrackerContract.methods.getAssetCount().call((error, response) => {
    if (error) console.log(error);
    else {
      assetCount = response;
      $("#count").html("Total " + response + " Drugs / Vaccines");
  ...
  async function renderTable() {
    for (let i = 1; i <= parseInt(assetCount); i++) {
      await AssetTrackerContract.methods.getAsset(i).call((error, response) => {
        if (error) console.log(error);
        else {
          let row =
            '<tr><th scope="row">' +
...
async function createNewAsset() {
  let userid = $('input[name="iduser"]').val();
  console.log("userid", userid)
  await AssetTrackerContract.methods.getIdentity(userid, account0)
    .call((error, response) => {
      if (error) {
         console.log(error)
         alert("Invalid user ID")
  ...
  // send these values to the smart contract
  await AssetTrackerContract.methods
    .createAsset(batchNo, name, desc, manufacturer, owner, quantity, status)
    .send()
    .then(result => {
      if (result.status === true) {
...
async function login() {
  ...
  console.log(id + password + account0);
  await AssetTrackerContract.methods.getIdentity(id, account0)
    .call((error, response) => {
      if (error) console.log(error);
      else {
        console.log(response[4]);
